...
// ---------------------------------------------------------------------
// 2 bis. L'ORDRE DES CODES — piège de JavaScript
// ---------------------------------------------------------------------
//
// Object.keys() ne rend PAS les codes dans l'ordre d'écriture. Les clés
// qui sont des entiers valides — '100', '101', '102' — sont énumérées
// AVANT les autres, parce que '010' à '090' portent un zéro initial et
// restent des chaînes. Conséquence observée le 14/09/2026 : la fiscalité
// sortait en tête de l'inventaire ET en tête des dossiers créés sur le
// Drive.
//
// Ne JAMAIS parcourir FAMILLES_DOC ou SOUS_FAMILLES par Object.keys()
// quand l'ordre compte. Passer par ces deux listes.

export const ORDRE_FAMILLES = Object.keys(FAMILLES_DOC).sort();
export const ORDRE_SOUS_FAMILLES = Object.keys(SOUS_FAMILLES).sort();
...
export function arborescence({ elaguer = [] } = {}) {
  const arbre = ORDRE_FAMILLES.map((fam) => ({
    nom: `${fam} ${FAMILLES_DOC[fam]}`,
    intangible: true,
    enfants: ORDRE_SOUS_FAMILLES
      .filter((code) => familleDe(code) === fam && !elaguer.includes(code))
      .map((code) => ({ nom: `${code} ${SOUS_FAMILLES[code].libelle}`, intangible: false })),
  }));
