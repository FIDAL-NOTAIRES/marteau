...
// axes de familles — LA source, versionnée avec le code (mémo v4 § 15 et
// § 21).
//
...
//
// À ARBITRER — trois rattachements sont une LECTURE de Claude, non une
// décision de JFD, et sont signalés ici plutôt que noyés dans la table :
//   - 021 → `usage` : la destination déclarée se lit au titre, et c'est
//     sa divergence avec l'usage réel qui sort en carmin. Non retenu
//     pour l'instant (le voyant est alimenté par 062).
//   - 061 → `zone` et `usage` : le PLU et le certificat d'urbanisme
//     disent le zonage et la destination admise. Retenu par défaut.
//   - 033 → `actif` : l'extrait de matrice est la pièce du test de
//     couverture de la taxe foncière. Retenu par défaut.
...
//
// SÉPARATEUR : l'espace, faute d'arbitrage. La fiche OUT 2.1.1 v1
// collait le groupe au code (« 0042OP »), ce qui devient illisible avec
// quatre blocs. À trancher avec JFD ; changer SEP suffit.
const SEP = ' ';
...
export function composerNom({ groupe, code, piece, date, autorisation }) {
  ...
  if (sf.parAutorisation) blocs.push(autorisation);
...
export function analyserNom(nom) {
  const blocs = String(nom).trim().split(/\s+/);
  if (blocs.length < 4) return { conforme: false, motif: 'moins de quatre blocs' };
